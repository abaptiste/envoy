#include "extensions/filters/network/udp/dns_filter/dns_filter_utils.h"
...
uint16_t BufferHelper::peekUint16(Buffer::Instance& buffer, uint64_t& offset) {
  ensureMaxLen(sizeof(uint16_t));

  const uint16_t val = buffer.peekBEInt<uint16_t>(offset);
